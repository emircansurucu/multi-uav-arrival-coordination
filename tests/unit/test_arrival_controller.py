...
def test_son_yaklasma_tavani_hizi_sinirlar():
    """Tavan verildiginde komut onun uzerine cikmamali.

    Azami hizda L1 kontrolcusunun donus yaricapi buyuyor; olculen sonuc
    hedefe 16 m yaklasabilmek ve 501 m rota sapmasi.
    """
    controller = make_controller()
    # Cok gec kalindi: tavan olmasa azami hiza cikardi.
    for _ in range(200):
        controller.update(600.0, 6000.0, planned_in(300.0), NOW_NS, 1.0,
                          max_airspeed_override=22.9)
    assert controller.commanded_airspeed_mps == pytest.approx(22.9, abs=0.01)


def test_tavan_deadband_icinde_de_uygulanir():
    """Zamanlama hatasi sifira yakinken bile yuksek hizda takili kalinmamali.

    Deadband dali komutu degistirmeden donuyordu; tavan indiginde arac
    azami hizda kalip son yaklasmayi bozuyordu.
    """
    controller = make_controller()
    # Once azami hiza cik.
    for _ in range(200):
        controller.update(600.0, 6000.0, planned_in(300.0), NOW_NS, 1.0)
    assert controller.commanded_airspeed_mps == pytest.approx(28.0, abs=0.01)

    # Simdi zamanlama tam tutuyor (deadband icinde) ve son yaklasmadayiz.
    komut = controller.update(300.0, 500.0, planned_in(300.0), NOW_NS, 1.0,
                              max_airspeed_override=22.9)
    assert komut.deadband_active is True
    assert komut.airspeed_mps == pytest.approx(22.9, abs=0.01)
    assert komut.changed is True, "tavan indi ama komut gonderilmiyor"


def test_tavan_asgari_hizin_altina_inemez():
    controller = make_controller()
    controller.update(300.0, 500.0, planned_in(300.0), NOW_NS, 1.0,
                      max_airspeed_override=5.0)
    assert controller.commanded_airspeed_mps >= 15.0
