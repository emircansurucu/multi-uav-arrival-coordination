...
Bağımlılığı yalnızca `std_msgs`'tır. Başka özel mesaj eklenmemiştir.
...
| Yol | İşlev |
|---|---|
| `run.sh` | **Tek giriş noktası** ortam hazırlığı, ön koşul denetimi, senaryo seçimi |
...
| `mission_manager.py` | 1200 | **Çekirdek.** 14 durumlu görev makinesi, 20 Hz kontrol döngüsü, çıpa uygulaması, kapı loiteri, terminal rezerv, robust E/L sınırları |
...
| Dosya | Satır | İşlev |
|---|---|---|
| `safety_manager.py` | **0** | **Boş.** `FAILSAFE` durumu enum'da tanımlı ama işleyici yok bilinen eksik |
...
Ayrıca **16 kavram sayfası** ([docs/kavramlar/](docs/kavramlar/)): her mekanizma
...
| S-manevrası | Uygulandı, uçuşta doğrulandı, **kaldırıldı** belge zorunlu tutmuyor, gerekçe [docs/04](docs/04-gelistirme-ve-testler.md) |
